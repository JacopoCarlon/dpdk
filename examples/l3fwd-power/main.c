...
#include <rte_epoll.h>			// for J hybrid
				...
				struct lcore_conf *qconf)
{
	...
	if (RTE_ETH_IS_IPV4_HDR(m->packet_type)) {
		...
		*/
		...
		// let's try to not hardcode it plz ... but needs 1 :(
		dst_port = 1;
...
- pmd-mgmt=monitor
		...
		-> which calls rte_power_monitor() 								in lib/eal/x86/rte_power_intrinsics.c
		-> which executes power_monitor_ops.mwait(tsc_timestamp);		in lib/eal/x86/rte_power_intrinsics.c
...
- pmd-mgmt=pause
		...
		rte_power_pause() 	is _tpause() 								in lib/eal/x86/rte_power_intrinsics.c
...
static int main_hybrid_loop(__rte_unused void *dummy)
{
	...
	while (!is_done()) {
		...
		*/
		...
		// Note:
		// 	on -> on -> off ... -> on -> drain -> on -> drain -> off -> hybrid logic . ok
		// 	on -> off -> off -> on -> drain -> off -> hybrid logic -> goto -> on -> drain -> off -> hybrid logic -> goto . ok!!
		// having the drain at the start allows to always cover a previous on operation.
		//	if the iteration was off, then we need not pass through drain and can use goto.
		cur_tsc = rte_rdtsc();
...
/*
remember !!! 
	from : lib/power/rte_power_pmd_mgmt.h 
...
static int
main_baselinePure_loop(__rte_unused void *dummy)
{
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
	unsigned int lcore_id;
	int i, j, nb_rx;
	...
	if (qconf->n_rx_queue == 0) {
		RTE_LOG(INFO, L3FWD_POWER, "lcore %u has nothing to do\n",
			lcore_id);
		printf("returning 0 from main_baselinePure_loop, since the number of receiver queues selected was 0.\n");
		return 0;
	}
	...
	while (!is_done()) {
		...
		*/
		...
		// run by: pure default baseline mode.
		// added rte_pause only if queues are all empty because that's how dpdk should be.
	...
	// since our test start the l3fwd before, and stops after, the actual packet transmission, 
	//		then we need to avoid overcounting pause-phase both before and after the busy-ish period.
	// - the final pause phase can simply be skipped by not updating it here. 
	// - the initial pause phase needs to be avoided by setting a flip flop variable (only flips once tho.)
...
/* main processing loop */
static int
main_pause_loop(__rte_unused void *dummy)
{
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
	unsigned int lcore_id;
	int i, j, nb_rx;
	...
	if (qconf->n_rx_queue == 0) {
		RTE_LOG(INFO, L3FWD_POWER, "lcore %u has nothing to do\n",
			lcore_id);
		printf("returning 0 from main_pause_loop, since the number of receiver queues selected was 0.\n");
		return 0;
	}
...
// jj 
static int
main_baselineWithNanoSecondPause_loop(__rte_unused void *dummy)
{
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
	unsigned int lcore_id;
	int i, j, nb_rx;
	...
	printf("\n\n --- --- main_baselineWithNanoSecondPause_loop: entered --- --- \n");
	...
	if (qconf->n_rx_queue == 0) {
		RTE_LOG(INFO, L3FWD_POWER, "lcore %u has nothing to do\n",
			lcore_id);
		printf("returning 0 from main_baselineWithNanoSecondPause_loop, since the number of receiver queues selected was 0.\n");
		return 0;
	}
...
main(int argc, char **argv)
{
	...
	if (app_mode == APP_MODE_LEGACY) {
		...
		rte_eal_mp_remote_launch(main_baselinePure_loop, NULL,
						SKIP_MAIN);
	} 
		...
		if (baseline_pause_enabled == true) {
			rte_eal_mp_remote_launch(main_baselineWithNanoSecondPause_loop, NULL, CALL_MAIN);
			printf("main _ chosen mode is : APP_MODE_PMD_MGMT baseline, with ns pause !!!! main_baselineWithNanoSecondPause_loop !!!! \n");
		}
